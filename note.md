...
- 実際に掘削する地点と、予測する地点を事前に決めておく
    - 掘削する地点は、最後の方に細かくすると良さそう
- 実際に掘削するかどうかをどう決めるか
    - 選択経路上の点の予測に必要な地点を掘削する
    - 各地点は、予測に必要な地点を保持しておく
- 最初の選択経路をどう決めるか
    - 実際に掘削する地点をあらかじめ調査しておく
- 追加で掘削する時は、周りの地点の頑丈度も参考にする
- 必要ないところは掘らない（K、Wが小さい時）
...
- 勾配を使いたい
- 頑丈度は正確にわからないので、うまく予測する

- とりあえず周りの地点との重みつき和で妥協
- 上位に行くには勾配を使うアイディアが欲しい
...
- 一個の家からの最短路を消して、新しい最短路を検索する
    - 連結じゃなくなった家を全て繋ぎ直す
    - 連結になるまで続ける
    - 探索する時には、上界が簡単にわかるので、それを上回らないようにする

## 2/23

つらいー

- 硬いところを開拓しきれていなくて、そこを使ってしまう
- 細かくすると、コストが大きくなってしまう
    - そもそも到達不可能なところを調べる必要ない
    - パスに含まれていないところを調べるのは無駄
...
- 各家から上限を決めたDFSをする
    - 評価関数が良いところを優先的に探索する
        - 水源までの距離と今までの距離
    - 何個も点を打つのは勿体無いので、近くの点を使う
- 開拓した道をもとに、各地点の硬さを決めて、焼きなましをする
    - estimated_gridをもとに
